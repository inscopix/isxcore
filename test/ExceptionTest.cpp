...
TEST_CASE("ExceptionTest", "[core]") {
    ...
    SECTION("Throw a DataIO exception with a message")
    {
        try
        {
            ISX_THROW(isx::ExceptionDataIO, "Expected: There was a DataIO error");
            FAIL("Failed to throw an exception");
        }
        catch (const isx::ExceptionDataIO& error)
        {
            REQUIRE(std::string(error.what()) == "Expected: There was a DataIO error");
        }
        catch (...)
        {
    ...
    SECTION("Throw a DataIO exception with a multi-string message")
    {
        try
        {
            ISX_THROW(isx::ExceptionDataIO, "Expected: Error with input ", 1);
            FAIL("Failed to throw an exception");
        }
        catch (const isx::ExceptionDataIO& error)
        {
            REQUIRE(std::string(error.what()) == "Expected: Error with input 1");
        }
        catch (...)
        {
            FAIL("Failed to throw an isx::ExceptionDataIO");
